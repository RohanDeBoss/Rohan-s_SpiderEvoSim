...
class Spider {
  ...
  float Ldist = MAX_LEG_SPAN * 0.25f;
  ...
  // Replaced ArrayList with HashSet for O(1) membership checks
  HashSet<Integer> swattersSeen = new HashSet<>();
  
  // Class fields for performance and caching
  ...
  color getColor() {
    ...
    } else if (logScale <= 0.08f) {
      float fac = logScale / 0.08f;
      newColor = color(0, 0, fac * 180, 255);
    } else if (logScale <= 0.16f) {
      float fac = (logScale - 0.08f) / 0.08f;
      newColor = color(0, fac * 50, 180, 255);
    } else if (logScale <= 0.24f) {
      float fac = (logScale - 0.16f) / 0.08f;
      newColor = color(0, 100, 180 - fac * 100, 255);
    } else if (logScale <= 0.32f) {
      float fac = (logScale - 0.24f) / 0.08f;
      newColor = color(fac * 60, 100 + fac * 60, 40, 255);
    } else if (logScale <= 0.40f) {
      float fac = (logScale - 0.32f) / 0.08f;
      newColor = color(128 + fac * 127, 220, 0, 255);
    } else if (logScale <= 0.48f) {
      float fac = (logScale - 0.40f) / 0.08f;
      newColor = color(255, 220 - fac * 92, 0, 255);
    } else if (logScale <= 0.56f) {
      float fac = (logScale - 0.48f) / 0.08f;
      newColor = color(255, 128 - fac * 128, 0, 255);
    } else if (logScale <= 0.64f) {
      float fac = (logScale - 0.56f) / 0.08f;
      newColor = color(255, fac * 20, fac * 147, 255);
    } else if (logScale <= 0.72f) {
      float fac = (logScale - 0.64f) / 0.08f;
      newColor = color(255 - fac * 127, 20 - fac * 20, 147 - fac * 19, 255);
    } else if (logScale <= 0.80f) {
      float fac = (logScale - 0.72f) / 0.08f;
      newColor = color(128 - fac * 128, fac * 255, 128 + fac * 127, 255);
    } else if (logScale <= 0.88f) {
      float fac = (logScale - 0.80f) / 0.08f;
      newColor = color(fac * 98, 255, 255 - fac * 111, 255);
    } else if (logScale <= 0.96f) {
      float fac = (logScale - 0.88f) / 0.08f;
      newColor = color(98 + fac * 157, 255 - fac * 40, 144 - fac * 144, 255);
    } else {
      float fac = (logScale - 0.96f) / 0.04f;
      newColor = color(255, 215 + fac * 40, fac * 255, 255);
    }
  ...
  void drawSpider(Room room) {
    ...
    float margin = MAX_LEG_SPAN * 1.5f;
  ...
  float getDarkestShadow() {
    ...
    for (Swatter sw : swatters) {
      ...
      for (int L = 0; L < LEG_COUNT && !legFound; L++) {
        float legX = leg_coor[L][0];
        float legY = leg_coor[L][1];
        if (legX >= minX && legX <= maxX && legY >= minY && legY <= maxY) {
          float dx = legX - swX;
          float dy = legY - swY;
          ...
          if (distSquared < R2) {
            ...
            darkest_sensed_shadow = Math.min(darkest_sensed_shadow, Math.max(sw.percentage, 0));
            legFound = true;
          }
        }
      }
  ...
  void randomShift() {
    ...
    float dangle = random(0.1f * PI, 1.9f * PI);
    for (int L = 0; L < LEG_COUNT; L++) {
